...
function ReviewContent({
  todayClients, todayDay, isClientReviewed, onCircleClick, navigate, currentSchedule,
}: {
  todayClients: Client[];
  todayDay: number;
  isClientReviewed: (id: string) => boolean;
  onCircleClick: (e: React.MouseEvent, client: Client) => void;
  ...
  return (
    <>
          ...
          <div className="space-y-2">
            {todayClients.map(client => {
              ...
              return (
                ...
                >
                  <button
                    onClick={(e) => onCircleClick(e, client)}
                  ...
                  <div className="flex-1 min-w-0">
                    <span className={`text-sm font-medium ${isDone ? 'line-through text-muted-foreground' : ''}`}>{client.name}</span>
      ...
      <div>
        <h2 className="text-sm font-syne font-bold text-muted-foreground mb-4 uppercase tracking-wider">Agenda da Semana</h2>
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 bg-secondary rounded-lg p-1 mb-6 w-fit">
        <button onClick={() => setTab('agenciado')}
