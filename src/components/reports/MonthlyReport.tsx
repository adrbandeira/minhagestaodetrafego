...
import { ChevronDown, CheckSquare, ClipboardList, Wallet, TrendingDown, Download } from 'lucide-react';
...
export default function MonthlyReportContent() {
  ...
  return (
    <div>
      ...
      {activeClients.length === 0 && <p className="text-muted-foreground text-sm">Nenhum cliente ativo.</p>}
          ...
          return (
            <div key={client.id} className="bg-card border border-border rounded-lg overflow-hidden">
              <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                ...
                <button onClick={() => exportClientPDF(client, data, monthLabel)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-border">
                <div className="p-5">
                  <h3 className="text-xs font-medium text-muted-foreground mb-3 flex items-center gap-1.5"><ClipboardList className="w-3.5 h-3.5" /> Revisões</h3>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Agendadas</span><span className="font-medium">{data.clientReviews.length}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Concluídas</span><span className="font-medium text-primary">{data.doneReviews}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Histórico registrado</span><span className="font-medium">{data.clientHistory.length}</span></div>
                ...
                <div className="p-5">
                  <h3 className="text-xs font-medium text-muted-foreground mb-3 flex items-center gap-1.5"><CheckSquare className="w-3.5 h-3.5" /> Tarefas</h3>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Total</span><span className="font-medium">{data.totalTasks}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Concluídas</span><span className="font-medium text-primary">{data.doneTasks}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Pendentes</span><span className="font-medium">{data.totalTasks - data.doneTasks}</span></div>
                    {data.totalTasks > 0 && (
                      <div className="mt-2">
                        <div className="w-full bg-secondary rounded-full h-2">
                ...
                <div className="p-5">
                  <h3 className="text-xs font-medium text-muted-foreground mb-3 flex items-center gap-1.5"><Wallet className="w-3.5 h-3.5" /> Verba de Anúncios</h3>
                      ...
                      {data.clientBalances.map(b => {
                        ...
                        return (
                          <div key={b.platform} className="space-y-1">
                            ...
                            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Saldo</span><span className="font-medium">R$ {b.balance.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
                            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Diário</span><span className="font-medium">R$ {b.daily_spend.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
